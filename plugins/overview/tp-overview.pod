...
Date Picker from http://sourceforge.net/projects/jscalendar/
...
Add the following lines to your apache config file (http.conf).
(Note: WEBPLAINDIR is /usr/local/torrus-xyz/web/plain by default):
...
=item * install torrus-plugin-overview
...
Also add the following lines:
    ...
    $Torrus::Renderer::Chooser::template = 'ov/ov-chooser.html';
