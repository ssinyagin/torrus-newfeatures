...
=item * C<gitlock:I<REPOPATH>> -- this key is used for the mutex that
  protects a local Git repository during pulls. Each consumer process
  that intends to make the pull, should acquire this mutex, then check
  again the latest coommit in its branch, and pull if it's not the
  latest one.
