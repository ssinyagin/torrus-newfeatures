...
values. Also the compiler addts an entry C<compiling:E<lt>tree_nameE<gt>>
...
B<snmp_lookup.db> is a I<Btree> database used by SNMP collector module.
It serves as a persistent storage for such SNMP data as interface names etc.

=over 4

=item * F: -- E<lt>formatE<gt>

Specifies the format version string. If the collector module's version
string is different from the one in the database, the database is automatically
flushed.

=item * M:E<lt>ipaddrE<gt>/E<lt>portE<gt>/E<lt>communityE<gt>
\0E<lt>mapE<gt>\0E<lt>keyE<gt> -- E<lt>indexE<gt>

Community may contain SNMPv3 user name. Each record stores an SNMP reverse
lookup for a table given in E<lt>mapE<gt> parameter. E<lt>keyE<gt> contains
the OID return value, and E<lt>indexE<gt> contains the index in the table.

=item * V:E<lt>ipaddrE<gt>/E<lt>portE<gt>/E<lt>communityE<gt>
\0E<lt>oidE<gt> -- E<lt>valueE<gt>

These records contain the SNMP values for given OIDs.

=item * B:E<lt>ipaddrE<gt>/E<lt>portE<gt>/E<lt>communityE<gt>
-- E<lt>timestampE<gt>

UNIX timestamp for the SNMP agent boot time.

=item * U:E<lt>ipaddrE<gt>/E<lt>portE<gt>/E<lt>communityE<gt>
-- E<lt>timestampE<gt>

UNIX timestamp of the last update for this SNMP agent.
The collector periodically re-scans the SNMP hosts,
and if the host information is too old it will be deleted.
