...
=item * C<words/I<TREE>/> contains subdirectories named after search
  keywords. These directories contain zero-length files named after node
  token IDs for nodes matching those keywords.

=item * C<wordsglobal/> is the same as above, but for global search.
