...
Tree node token starts with letter I<T>. The rest is the 40-character
SHA-1 checksum of a node as described above.
...
A Torrus instance uses two working directories for each tree configuration:

=over

=item * The XML compiler's working directory is used exclusively by the
  compiler. Once the compiler finishes its work, it commits the changes
  and (optionally) pushes them to the repository. Then it sends a
  notfication to Redis, so that the consumer processes can update their
  data.
...
=item * The working directory of consumer processes on a given host
  ...
  first consumer that needs the configuration, is checking the
  update status in Redis. If there are new commits, it sets an exclusive
  lock on the working directlry via Redis, and pulls the changes from
  the repository. If both compiler and the consumer are sharing the same
  host, the compiler's working directory can act as a repository, thus
  saving on network communication.

=back

Each tree has its own pair of working directories, so the total number
of Git repositories is equal to the number of trees.

The working directory has subdirectories as follows. The directories
C<nodes>, C<children>, and C<agents> contain JSON files named after the
...
=item * C<agents/I<DAEMON>/I<TREE>/I<INSTANCE>/>: each daemon that needs
  a quick access to a subset of leaf nodes (primarily, I<collector>, and
  also I<monitor>) can retrieve the node configurations from this
  structure. The instance number is 4-digit lower-case hexademical
...
allowing multiple Torrus installation to use the same Redis
...
=item * C<wdlock:I<WDPATH>> -- this key is used for the mutex that
  protects the consumer working directory during the Git pull
  operation. Each consumer process that intends to make the pull, should
  ackquire this mutex, then check the actual HEAD of the working
  directory, and pull if it's not the latest one.

=item * C<treehead:I<TREE>> -- the value is the Git commit number of the
  compiler's working directory. The consumer process compares this with
  the consumer working directory's commit and pulls the updates if these
  commit numbers are different.

=item * C<tsets:I<TREE>> -- list of tokenset names.

=item * C<tset:I<TREE>:I<TSET>> -- hash of tokenset members. Tokens are
the keys, and the values are "S" for static, "M" for monitor, indicating
their origins.
...
=item * C<treecommits:I<TREE>> -- the value of every new Git commit is
