...
E<lt>t_last_changeE<gt>[:E<lt>escalationE<gt>[:E<lt>escalationE<gt>...]]
...
timestamp that shows when it's still important to keep the entry after
the alarm is cleared. B<status> is 1 if the alarm is active, and 0
otherwise.  B<t_last_change> is the timestamp of last status
change. Following values are optional escalation times when escalation
events were fired.
