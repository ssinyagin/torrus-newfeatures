...
and the JSON files contain JSON arrays with agent branch names where a
...
=item * C<srcglobaldeps>: a JSON hash representing the XML source files
  which, if changed, enforce a full rebuild of the tree
  configuration. The keys are file names, and values are "1".

=item * C<srcrev>: A file containing a JSON scalar referring to the
  commit in XML sources.

=item * C<srcfiles>: A file containing a JSON hash with source XML files
  as keys and "1" as values.
...
Optionally, Memcached is used to store various kinds of temporary data
for quick access. Each key in the below list is prepended by a prefix
specific to this Torrus installation ("torrus:" by default), tree name,
and the 4-symbol commit ID of the configuration tree, separated by
colons.
...
=item * C<np:I<NAME>:I<PNAME>> -- stores a calculated value of a
  ...
  value. This cache is turned off by default, as it's adding a delay.
