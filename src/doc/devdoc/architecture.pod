...
A Torrus instance uses a number of working directories for each tree
configuration:

=over

=item * The XML compiler's working directories are used exclusively by
  the compiler. Once the compiler finishes its work, it commits the
  changes and (optionally) pushes them to the repositories. Then it
  sends a notfication to Redis, so that the consumer processes can
  update their data.

=item * The working directory of consumer processes on a given host
  (there might be multiple hosts sharing the same configuration). The
  consumer can check the update status in Redis and pull the new commits
  if necessary. Before pulling, it sets an exclusive lock on the working
  directlry via Redis, and pulls the changes from the repository. If
  both compiler and the consumer are sharing the same host, the
  compiler's working directory can act as a Git remote, thus saving on
  network communication.

=back

Each tree has its own set of working directories. The C<configtree>
working directory contains a full hierarchy of objects, so all
parameters that are defined in the input XML are retrievable. Typically
the Web UI renderer is consuming this data. Also there's a number of
agent working directories, one per agent instance (collectors and
monitors are typical agents). These working directories contain only the
information and parameters needed by the agents, so that they can start
and update their data as fast as possible.

The C<configtree> working directory has subdirectories as follows. The
...
=item * C<children/>: tor each subtree node, there is a JSON file
  defining an array of child tokens.
...
=item * C<nodeid/>: two-level 256-way directory structure. Each file is a
  SHA-1 digest of the tree name, followed by semicolon, and the
  I<nodeid> value. The content of the file is a JSON scalar indicating
  the node token.
...
=item * C<other/>: JSON objects for views, monitors, and actions
  ...
  and the content is a hash with parameters defining each object.
...
In large installations, the number of files can exceed 1 million, and
filesystem optimization might be needed. Ext4 is fixed to the maximum
number of inodes at creation time. Btrfs is thought to be a better
match, and it looks to be better optimized for large numbers of small
files.
...
=item * C<tree>: the tree name

=item * C<type>: "S" for subtree, "L" for a leaf.

=item * C<parent>: token of the parent node, or empty string if this is
the top of the tree.

=item * C<name>: the node name. Subtree names must end with slash, and
leaf names should end with alphanumeric characters.
...
The agent working directories are stored at
C<agents/I<DAEMON>_I<INSTANCE>/> path and contain JSON files named after
...
Git branch names are fixed as follows: the C<configtree> directory is in
branch C<I<TREE>_configtree>, and agent working directories are in
branches C<I<TREE>_II<DAEMON>_I<INSTANCE>>. This naming allows all
branches to be pushed into a single remote repository in case if
...
=item * C<wdlock:I<WDPATH>> -- this key is used for the mutex that
  protects a working directory during updates (compiler uses this for
  guaranteeing that only one process is working at a time, and consumers
  use this during Git pull operation). Each consumer process that
  intends to make the pull, should acquire this mutex, then check the
  actual HEAD of the working directory, and pull if it's not the latest
  one.

=item * C<githeads:I<TREE>> -- this is a hash containing the commit
  numbers written by the compiler. The keys are C<configtree> and
  C<agents/I<DAEMON>_I<INSTANCE>/>, and the values are the Git commit
  numbers of corresponding repositories. The consumer process compares
  this with the current known commit and pulls the updates if needed.

=item * C<tsets:I<TREE>> -- list of tokenset names.

=item * C<tset:I<TREE>:I<TSET>> -- hash of tokenset members. Tokens are
  the keys, and the values are "S" for static, "M" for monitor,
...
=item * C<treecommits:I<TREE>> -- the value of every new Git commit in
  C<configtree> repository is published to this channel.
...
=item * C<np:I<NAME>:I<PNAME>> -- stores a calculated value of a
  parameter for a given node token or view. The value a concatenation of
  status character and the parameter value. The status character is C<U>
  for undefined parameter, and C<D> for a parameter with value.
