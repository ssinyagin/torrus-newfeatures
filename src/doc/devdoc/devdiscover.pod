...
=head1 RRFW SNMP Device Discovery Developer's Guide
...
See I<RRFW Command Reference> for command usage and functionality overview.
...
This file is installed as C<bin/devdiscover> in RRFW installation directory,
...
verified, the control is passed to an C<RRFW::DevDiscover> object.

=item * C<RRFW::DevDiscover>
...
initiates a new C<RRFW::DevDiscover::DevDetails> object for the target host;
...
=item * C<RRFW::DevDiscover::DevDetails>

This Perl module is defined in F<lib/RRFW/DevDiscover.pm>, and provides
the functionality to store the results of SNMP device discovery.

=item * C<RRFW::ConfigBuilder>
...
as C<RRFW::DevDiscover::RFCXXXX_SOME_HUMAN_NAME>, and vendor-specific
ones are named as C<RRFW::DevDiscover::VendorProduct[Subsystem]>.

=item * Template definition files

These are XML documents residing in F<xmlconfig/vendor> and
F<xmlconfig/generic> directories. Each file is a piece of RRFW configuration,
...
I<RRFW Programming Style Guide>.

Upon initialization, C<RRFW::DevDiscover> loads the modules listed in
C<@RRFW::DevDiscover::loadModules> array. This array is pre-populated
by standard module names in F</share/rrfw/devdiscover-config.pl>.
You can add new module names by pushing them onto this array in your
local F</share/rrfw/devdiscover-siteconfig.pl>.
...
Example:
    ...
    $RRFW::DevDiscover::registry{'RFC2790_HOST_RESOURCES'} = {
        'sequence'     => 100,
...
Exceptional is C<RFC2863_IF_MIB> module, which has the sequence number 10.
That is because it provides the basic interface discovery,
and many other modules depend on its results.

=item * C<checkdevtype>

Must be a subroutine reference. This subroutine is called with two
arguments: C<RRFW::DevDiscover> and C<RRFW::DevDiscover::DevDetails>.
...
variable. The C<RFC2863_IF_MIB> module is an exception, because it tries
...
whose C<checkdevtype()> returned true. The subroutine should return true
...
references as arguments: C<RRFW::DevDiscover::DevDetails>,
C<RRFW::ConfigBuilder>, and an XML element object, which should be used only
...
F</share/rrfw/devdiscover-config.pl> or in
F</share/rrfw/devdiscover-siteconfig.pl>.
...
template name in the global hash C<%RRFW::ConfigBuilder::templateRegistry>
and figures out the source XML file and the actual template name.

Example:

    $RRFW::ConfigBuilder::templateRegistry{
...
C<RRFW::DevDiscover::RFC2863_IF_MIB> provides the functionality to
...
C<RRFW::DevDiscover::RFC2863_IF_MIB::addInterfaceFilter
