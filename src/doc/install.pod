...
lines to your F<$Torrus_HOME/share/torrus/torrus-siteconfig.pl>:
...
Perl version C<5.8.0> or higher is required. Version C<5.8.0> is not
...
is used for all data storage. Versions C<4.2.52> and above are recommended.
...
Unfortunately, Perl module C<BerkeleyDB> cannot find the include and
library files in that directory. So, it either needs C<--prefix=/usr/local>
configure option, or you need to edit F<config.in> in C<BerkeleyDB>
distribution.

=item * HTTP Server

This software requires Apache HTTP server (E<lt>http://httpd.apache.orgE<gt>)
...
Minimum supported mod_perl 2.0 version is B<1.99_15>. The handler also requires
...
SNMP queries and traps interface. Version C<4.0.3> is required.
...
It is available from CPAN E<lt>http://search.cpan.org/E<gt>, and only in
...
Create the group C<torrus> and a user C<torrus> belonging to that group.
...
The package is installed by default in the directory
F</usr/local/torrus-X.Y> where X.Y.Z is the release version.
Further on, we reference this directory as
F<$Torrus_HOME>.

Prerequisite Perl modules are checked at the time of C<./configure>. You
can disable this by giving I<--disable-modcheck> option.

The only files that are not overwritten during C<make install> are
F<$Torrus_HOME/share/torrus/torrus-siteconfig.pl> and
F<$Torrus_HOME/share/torrus/xmlconfig/site-global.xml>.

The following directories and their contents must be writable by your Torrus
processes, including the HTTP server (usually Apache web server):

=over 4

=item * F<$Torrus_HOME/var/cache/>

The placeholder directory for Renderer cache files. These are PNG and HTML
files ready for display by Apache handler.

=item * F<$Torrus_HOME/var/db/>

The home directory for Berkeley DB environment. Here all the databases are
stored.

=item * F</var/snmpcollector/>

This is the default directory for SNMP collector RRD files.
It may be changed with the I<--datadir> option of C<devdiscover>.
...
Default values for a non-Cygwin OS are: I<var_user=torrus>,
...
changing the I<--localstatedir=DIR> option of C<./configure>.
...
Before running C<./configure>, unpack the plugin distribution archives into
some directory:
...
option I<--with-plugins> of C<./configure>:

  ./configure [VARIABLE=VALUE...] \
    --with-plugins=../torrus-plugin-ABC-0.0.1:../torrus-plugin-XYZ-0.0.1 \
    [other options...]

After that, C<make install> will install Torrus together with the specified
plugins.

From the Torrus home page, you may download an example plugin called
I<torrus-plugin-dummy>, and use it as a template for your own add-ons.

=head2 Configuring Torrus

The datasources are configured with C<%Torrus::Global::treeConfig>
hash in F<$Torrus_HOME/share/torrus/torrus-siteconfig.pl>.
...
I<xmlfiles> points to an array
of source XML files; I<run> points to a hash with the names of the daemons
...
By default, only F<defaults.xml> and F<site-global.xml> need to be
included in compilation. The XML files with vendor and MIB definitions
are automatically included by XML files produced with C<devdiscover>.
...
products. C<devdiscover> includes some of these files automatically into
configuration.
...
In addition, the directory F<$Torrus_HOME/share/torrus/xmlconfig/examples>
...
See F<$Torrus_HOME/share/torrus/torrus-config.pl> for the complete list
...
=item * C<$Torrus::Renderer::stylingProfile>
...
of the profile with F<.pl> suffix. The file is normally resided in
F<$Torrus_HOME/share/torrus/styling> directory.
See F<$Torrus_HOME/share/torrus/styling/torrus-original.pl> for reference and
examples.
...
to zero in your F<$Torrus_HOME/share/torrus/torrus-siteconfig.pl>.
...
F<$Torrus_HOME/bin/cleanup> in a cron job, once per day:
...
Stop the collector and monitor processes (usually by C</etc/init.d/torrus stop>).
