...
C<genddx> is designed as a one-time utility. You may run it to create
a typical discovery file with basic set of options. Then the discovery
configuration XML woild be the primary source of information, and it should
...
=head2 C<torrus devdiscover>: SNMP discovery tool
  ...
  Usage: torrus devdiscover --in=filename.xml options...
  Options:
