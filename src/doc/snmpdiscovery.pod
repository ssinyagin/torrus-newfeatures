...
The discovery tools consist of two programs: C<devdiscover> performs
...
Another utility, C<genddx>, is a program that generates the basic discovery
instructions file based on a set of commandline options.


=head2 C<genddx>: Discovery instructions generator
...
This utility generates C<devdiscover> instructions XML file based on
commandline options and a plain list of SNMP agents' hostnames.
Hostnames are apecified with one or many C<--host=hostname> options,
...
Examples:
  ...
  ./bin/genddx --out=ch-langenthal.ddx \
  ...
  ./bin/devdiscover --in=ch-langenthal.ddx



=head2 C<devdiscover>: SNMP discovery tool
...
Output file name is taken from C<output-file> parameter in the instructions
XML file. If the output file is not an absolute path, the file is placed
in the site XML configuration directory.
...
F<$Torrus_HOME/share/torrus/devdiscover-siteconfig.pl> file. Default values for
those variables reside in F<$Torrus_HOME/share/torrus/devdiscover-config.pl>.
...
If no absolute path given, the default site XML configuration directory
is used.
...
=item * C<snmp-port>, C<snmp-community>,  C<snmp-version>, C<snmp-timeout>,
        C<snmp-retries>, C<snmp-host>
...
=item * C<RFC2863_IF_MIB::errors-monitor>

When set to C<yes>, all interface error and discard counters are assigned to
a monitor called C<RFC2863_IF_MIB-errors>. It is up to you to define the
...
=item * C<CiscoIOS::disable-membuf-stats>

When set to C<yes>, Cisco memory buffer statistics are excluded from
discovery.
