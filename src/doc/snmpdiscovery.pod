...
By default, C<genddx> specifies the Torrus configuration output file
with absolute path pointing to C<xmlconfig> subdirectory of the installation,
with the file name <routers.xml>. You most probably will change this by
using the C<--discout> option.
...
In the parameter value, C<$XMLCONFIG> is replaced with the default Torrus XML
