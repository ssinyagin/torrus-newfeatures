...
=item * Required Styles  (line-style,line-color)
...
=item * Generic Symbolic styles (line-color only)

                    in                   out

=item * Symbolic, Symbolic names, for default use in graphs that have up
to ten items. (line-color only)
...
=item * Symbolic names for combinatorial graphing. (line-color,line-style)
...
=item * Color definitions from the TT2 rgb example set. (line-color only)

Defined in F<styling/colornames.pl>
...
=item * Color definitions for web html colors. (line-color only)
...
located in the C<styling> directory.
...
In the torrus-siteconfig.pl file, add the variable
  ...
  $Torrus::Renderer::stylingProfileOverlay = 'fish-schema';
