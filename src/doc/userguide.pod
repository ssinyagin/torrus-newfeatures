...
you would find them in F</usr/local/etc/torrus/discovery/>.
The result of C<devdiscover> is the Torrus configuration files:
F<myrouters.xml> and F<myswitches.xml>, placed into
F</usr/local/etc/torrus/xmlconfig/>. The C<collector> will place the
RRD files into F</data1/snmpcollector>. Make sure that this directory
exists, has enough free space, and is writable by C<torrus> user. B<Note:>
the C<genddx> utility is designed as a one-time helper, so that you create
...
F</usr/local/etc/torrus/conf/torrus-siteconfig.pl>.
...
F</var/log/torrus/collector.treename.log>.
