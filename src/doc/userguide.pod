...
all prerequisits and necessary steps are described there. By default,
the software will be installed in F</usr/local/torrus-0.1> (as long
as the major release version is 0.1). Further in this document,
we reference this path as a hypothetical variable F<$Torrus_HOME>.

B<What is where>. The executables may be surprisingly found in
F<$Torrus_HOME/bin/>. All site-specific behaviour is controlled by
F<$Torrus_HOME/share/torrus/torrus-siteconfig.pl>. In this file, you
must list your XML configuration sources. All the configuration is
read out of XML files resided in F<$Torrus_HOME/share/torrus/xmlconfig/>.
Site-specific global XML configuration parameters may be defined in
F<$Torrus_HOME/share/torrus/xmlconfig/site-global.xml>.
XML configuration is compiled into internal database representation
by F<$Torrus_HOME/bin/compilexml>. The database itself is resided in
F<$Torrus_HOME/var/db/>, and must be writable by your Apache server
(normally the installer takes care of it). It is safe to re-compile the
configuration while the other daemons are running.

B<The datasource trees>. Torrus configuration consists of a number of I<trees>.
Each tree is independent from the others. Each of them may or may not run
a separate Collector or Monitor process. Web interface access control lists
...
Subtree contains child subtrees and/or leaves. Leaf represents a datasource:
normally this is a numerical value that changes over time. The leaf is
the entity that may be presented as a graph. There are leaves of special
type: I<multigraph>. They are not numerical values, and are designed for
...
B<Trees configuration>. The trees are defined in
F<$Torrus_HOME/share/torrus/torrus-siteconfig.pl>. See I<Torrus Installation
Instructions> for a basic example of tree configuration.
...
C<$Torrus_HOME/bin/collector>, or by some other external programs.

B<Define the targets>. If you only want to collect SNMP counters
from some network devices' interfaces, there's a copule of tools
called C<$Torrus_HOME/bin/genddx> and C<$Torrus_HOME/bin/devdisover>.
The first one creates a basic discovery instructions file, and the second
one uses the discovery instructions to explore the device capabilities
and information: interface names, input/output counters,
CPU and memory usage, temperature sensors (Cisco devices).
...
B<Build the hierarchy>. By default, C<genddx> will put all your
...
of C<genddx>. This program may also read the device names (or IP addresses
in case if you don't use DNS) from space-delimited text files.

  cd /usr/local/torrus-0.1
  ./bin/genddx \
    --hostfile=share/torrus/myrouters.txt \
    --domain=example.net \
    --community=MySecretSNMPCommunity \
    --out=share/torrus/discovery/myrouters.ddx \
    --discout=share/torrus/xmlconfig/myrouters.xml \
    --subtree=/My_Routers \
    --datadir=/data1/snmpcollector

  ./bin/genddx \
    --hostfile=share/torrus/myswitches.txt \
    --domain=example.net \
    --community=MySecretSNMPCommunity \
    --out=share/torrus/discovery/myswitches.ddx \
    --discout=share/torrus/xmlconfig/myswitches.xml \
    --subtree=/My_Switches \
    --datadir=/data1/snmpcollector

  ./bin/devdiscover  --in=share/torrus/discovery/myrouters.ddx

  ./bin/devdiscover  --in=share/torrus/discovery/myswitches.ddx

In the example above, the routers' and switches' names are read from
F<myrouters.txt> and F<myswitches.txt>. They form a hierarchy with
two subtrees: C</My_Routers/> and C</My_Switches/>.
C<genddx> creates the discovery instruction XML files into
F<myrouters.ddx> and F<myswitches.ddx> accordingly.
The result of C<devdiscover> is the Torrus configuration files: F<myrouters.xml>
and F<myswitches.xml>. The C<collector> will place the RRD files into
F</data1/snmpcollector>. Make sure that this directory exists and has enough
free space.

B<Add your XML files to the tree configuration>.  Next step would be to
add F<myrouters.xml> and F<myswitches.xml>
into C<xmlfiles> array in the tree configuration in your
F<$Torrus_HOME/share/torrus/torrus-siteconfig.pl>.
...
This might be arranged by geographical locations, or by customer names.
...
B<Compile the configuration>. Now that your configuration is
ready, you need to execute the compiler:

  ./bin/compilexml --tree=treename --verbose

For most of the processes that you run within Torrus, you need to specify
the tree name with C<--tree> option. Some prorams accept C<--all> option,
...
F<$Torrus_HOME/var/log/collector.treename.log>.

There is a file that is created by C<make install>, F<init.torrus>.
...
on FreeBSD). Most probably, you need to rename and edit the script
before using.
Note that it also executes another daemon, C<monitor>. You will read
later about it in more details.

B<Define the ACLs>. By default, user authentication is enabled in the web
interface. You can change this by setting
C<$Torrus::ApacheHandler::authorizeUsers = 0> in your F<torrus-siteconfig.pl>.
...
installation guide to the
end, and your HTTP server is running, your Torrus hierarchy must be visible
...
chid XML elements of a C<E<lt>subtreeE<gt>> element define the child
...
C<devdiscover> will extract some useful information from your SNMP devices,
and place it in the XML configuration:
...
instructions would be like
  ...
  ./bin/devdiscover \
     --in=share/torrus/discovery/routers.ddx \
     --out=share/torrus/xmlconfig/routers.xml

  xupdate -j share/torrus/examples/xupdate/setmonitor.xup \
    share/torrus/xmlconfig/routers.xml > share/torrus/xmlconfig/routers1.xml
...
Torrus distribution has a special-purpose XSLT template, F<extract-skeleton.xsl>,
designed to strip all parameters and template applications from a given
XML configuration, and leave the tree structure only. Given that
F<share/torrus/xmlconfig/routers.xml> is some autogenerated configuration,
you may run

  xsltproc share/torrus/sup/extract-skeleton.xsl \
    share/torrus/xmlconfig/routers.xml  | \
...
the number of homogenious devices is more than one.
...
the TorrusQ configuration file:
...
This is the installation directory of Torrus. All other manuals refer it as
C<$Torrus_HOME>.

=item * F</usr/testtorrus/snmpcollector>
...
=item * Installation procedure
  ...
  gzip -dc torrus-0.1.5.tar.gz | tar xvf -
  cd torrus-0.1.5
  ./configure --prefix=/usr/testtorrus/home
  make install

=item * Devdiscover configuration

Use devdiscover as usual. Place your discovery instruction files in
F</usr/testtorrus/home/share/torrus/discovery/>, and make sure that the
C<output-file> points inside F</usr/testtorrus/home/share/torrus/xmlconfig/>,
and C<data-dir> is set to F</usr/testtorrus/snmpcollector>.
...
given below:
  ...
  <VirtualHost *:8123>
    ...
    PerlRequire "/usr/testtorrus/home/share/torrus/webmux.pl"
    <Location />
...
Copyright (c) 2002-2003 Stanislav Sinyagin E<lt>ssinyagin@yahoo.comE<gt>
